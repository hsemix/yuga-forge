...
### Supporting method on Resource

You must **not** call anything extra in your Resource — Forge calls these automatically:

```php
// These are generated by Forge internally — shown here for reference only.
public function selectRelationOption(string $field, string $id): void { ... }
```

The combobox's search text lives in `$this->relationSearch[$name]` (cleared on selection or form reset).
